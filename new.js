...
// ============================================
// SUNWIN ULTIMATE AI - FULL 500+ THUẬT TOÁN
// ============================================
...
// Load lịch sử từ DB khi khởi động
db.all("SELECT * FROM sessions ORDER BY phien ASC", (err, rows) => {
    if (err) {
        console.error("Lỗi load DB:", err.message);
        return;
    }
    for (const row of rows) {
        sunwinAI.addSession({
            ket_qua: row.ket_qua,
            tong: row.tong,
            xuc_xac_1: row.xuc_xac_1,
            xuc_xac_2: row.xuc_xac_2,
            xuc_xac_3: row.xuc_xac_3,
            phien: row.phien,
            thoi_gian: row.thoi_gian
        });
    }
    console.log(`📚 Đã tải ${sunwinAI.history.length} phiên từ database`);
});
...
app.get('/api/dudoan', (req, res) => {
    ...
    // Lấy phiên hiện tại lớn nhất
    db.get("SELECT MAX(phien) as max_phien FROM sessions", (err, row) => {
        const currentPhien = row?.max_phien || 0;
        const nextPhien = currentPhien + 1;
        
        // Lấy lý do từ topSources
        let lyDo = "";
        if (prediction.topSources && prediction.topSources.length > 0) {
            lyDo = prediction.topSources.map((s, i) => 
                `${i+1}. ${s.source} (${s.confidence}%) → ${s.prediction}`
            ).join("; ");
        } else {
            lyDo = `Tổng hợp từ ${prediction.totalModels} thuật toán AI`;
        }
        
        // Thêm thông tin đồng thuận
        if (prediction.top50Agree) lyDo += "; Đồng thuận TOP50 mạnh";
        else if (prediction.top20Agree) lyDo += "; Đồng thuận TOP20";
        else if (prediction.top10Agree) lyDo += "; Đồng thuận TOP10";
        ...
        res.json({
            ...
            ly_do: lyDo,
            update_at: new Date().toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }),
            chi_tiet: {
                do_tin_cay: prediction.confidence,
                so_thuat_toan: prediction.totalModels,
                top10_dong_thuan: prediction.top10Agree,
                top20_dong_thuan: prediction.top20Agree,
                top50_dong_thuan: prediction.top50Agree,
                do_chinh_xac_lich_su: `${stats.accuracy}%`,
                so_phien_da_phan_tich: sunwinAI.history.length
            }
        });
    });
});
...
// ============================================
// API: LẤY DỮ LIỆU TỪ NGUỒN VÀ LƯU DB
// ============================================
async function fetchAndSaveData() {
    ...
    try {
        ...
        db.get("SELECT phien FROM sessions WHERE phien = ?", [phien], (err, row) => {
            if (err) {
                console.error("Lỗi kiểm tra phiên:", err.message);
                return;
            }
            ...
            if (!row) {
                // Chèn phiên mới
                db.run(
                    `INSERT INTO sessions (phien, ket_qua, thoi_gian, tong, xuc_xac_1, xuc_xac_2, xuc_xac_3)
                     VALUES (?, ?, ?, ?, ?, ?, ?)`,
                    [phien, ketQua, thoiGian, tong, x1, x2, x3],
                    (err2) => {
                        if (err2) {
                            console.error("Lỗi insert DB:", err2.message);
                        } else {
                            console.log(`✅ Đã lưu phiên ${phien}: ${ketQua} (${tong})`);
                            
                            // Thêm vào AI history
                            sunwinAI.addSession(data);
                            
                            // Feedback nếu có dự đoán trước đó
                            if (sunwinAI.predictions.length > 0) {
                                const lastPrediction = sunwinAI.predictions[sunwinAI.predictions.length - 1];
                                if (lastPrediction.prediction === ketQua) {
                                    console.log(`🎯 Dự đoán CHÍNH XÁC! (${lastPrediction.prediction})`);
                                } else {
                                    console.log(`❌ Dự đoán SAI. (Dự đoán: ${lastPrediction.prediction}, Thực tế: ${ketQua})`);
                                }
                ...
                console.log(`⏭️ Phiên ${phien} đã tồn tại, bỏ qua`);
            }
        });
...
// ============================================
// KHỞI ĐỘNG SERVER & CHẠY LẤY DỮ LIỆU ĐỊNH KỲ
// ============================================
app.listen(PORT, () => {
    console.log(`
╔══════════════════════════════════════════════════════════╗
║     🚀 SUNWIN ULTIMATE PREDICTOR - PORT ${PORT}            ║
║     📊 500+ THUẬT TOÁN AI - DỰ ĐOÁN CHUẨN NHẤT           ║
╠══════════════════════════════════════════════════════════╣
║  API:                                                     ║
║    GET http://localhost:${PORT}/api/dudoan  → Dự đoán     ║
╚══════════════════════════════════════════════════════════╝
    `);
